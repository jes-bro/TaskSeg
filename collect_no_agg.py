...
def generate_pseudo_gt(demo, keypoint_1, keypoint_2, camera_view):
    ...
    if camera_view == 'front':
        obs_prev = demo[frame_range.start].front_rgb
    elif camera_view == 'left_shoulder':
        obs_prev = demo[frame_range.start].left_shoulder_rgb
    elif camera_view == 'right_shoulder':
        obs_prev = demo[frame_range.start].right_shoulder_rgb
    elif camera_view == 'overhead':
        obs_prev = demo[frame_range.start].overhead_rgb

    # Iterate over the frame range starting from the second frame
    for idx in range(frame_range.start + 1, frame_range.stop):
        if camera_view == 'front':
    ...
    # Initialize the first frame again
    if camera_view == 'front':
        obs_prev = demo[frame_range.start].front_rgb
    elif camera_view == 'left_shoulder':
        obs_prev = demo[frame_range.start].left_shoulder_rgb
    ...
    for idx in range(frame_range.start + 1, frame_range.stop):
        ...
        # Use RAFT to get optical flow
        optical_flow = get_pred_flow_sequence_normalized(obs_prev, obs_n, max_flow)
        ...
        category_name = f"WINE-same-{camera_view}-{frame_range.start}-{fstop}"
        ...
        ground_truth_seg_loose = (norm_mag_sum >= 0.0001).astype(np.uint8)
        ground_truth_seg_tight = (norm_mag_sum >= 0.0001).astype(np.uint8)
