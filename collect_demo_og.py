...
breakpoint()
from RAFT3D.raft3d.raft3d import RAFT3D
...
def generate_pseudo_gt(demo, keypoint_1, keypoint_2, camera_view):
    ...
    for idx in frame_range:
        ...
        # deviation = (mag_no_robot - mean) / std
        ...
        category_name = f"safe-framewise-normalization-sequence-outlier-removal{camera_view}-{frame_range.start}-{fstop}"
    ...
    ground_truth_seg_loose = (norm_mag_sum_clipped >= 0.001).astype(np.uint8)
    ground_truth_seg_tight = (norm_mag_sum_clipped >= 0.05).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    # Set up RAFT model
    raft_model = torch.nn.DataParallel(RAFT3D(args))
