...
def generate_pseudo_gt(demo, keypoint_1, keypoint_2, camera_view):
    if camera_view == 'front':
        ...
        gt_mask = demo[keypoint_1].front_mask.copy()
        ...
        gt_mask = demo[keypoint_1].left_shoulder_mask.copy()
        ...
        gt_mask = demo[keypoint_1].right_shoulder_mask.copy()
        ...
        gt_mask = demo[keypoint_1].overhead_mask.copy()
    ...
    robot_seg = gt_mask.copy()
    robot_seg[robot_seg>=48] = 1
    robot_seg[robot_seg<30] = 1
    robot_seg[robot_seg!=1] = 0
    mag_sum = np.zeros((obs_1.shape[0], obs_1.shape[1]))
    # breakpoint()
    umbrella_seg = gt_mask.copy()
    umbrella_seg[umbrella_seg == 85] = 1
    umbrella_seg[umbrella_seg == 84] = 1
    # umbrella_seg[umbrella_seg == umbrella_seg.max()] = 1
    umbrella_seg[umbrella_seg!=1] = 0
    # breakpoint()
    ...
    category_name = f"NAIL-same-{camera_view}-{frame_range.start}-{fstop}"
    value = 0
    ...
    umbrella_save_dir = os.path.join('tsg', 'umbrella_seg', category_name) 
    os.makedirs(umbrella_save_dir, exist_ok=True) 
    np.save(os.path.join(umbrella_save_dir, f'{value:05}.npy'), umbrella_seg)
    plt.imsave(os.path.join(umbrella_save_dir, f'{value:05}.jpg'), umbrella_seg)
    ...
    for idx in frame_range:
        ...
        obs_n2 = resize(obs_n, (400, 400), preserve_range=True, anti_aliasing=True)/255
        ...
        flow_resized = resize(optical_flow.cpu().numpy(), (400, 400), preserve_range=True, anti_aliasing=True)
        ...
        os.makedirs(flow_save_dir, exist_ok=True) 
        flow = flow_to_image(optical_flow.cpu().numpy())
        flow2 =  resize(flow, (400, 400), preserve_range=True, anti_aliasing=True) / 255
        plt.imsave(os.path.join(flow_save_dir, f'{value:05}.jpg'), flow)
        plt.clf()

        # breakpoint()

        obs1_resized = cv2.resize(obs_1, (400, 400), interpolation=cv2.INTER_CUBIC)
        rgb_save_dir = os.path.join('tsg', 'flowsam_jpgs', category_name)
        os.makedirs(rgb_save_dir, exist_ok=True)
        plt.imsave(os.path.join(rgb_save_dir, f'{0:05}.jpg'), obs1_resized)
        robot_seg2 = cv2.resize(robot_seg, (400, 400), interpolation=cv2.INTER_CUBIC)
        np.save(os.path.join(rgb_save_dir, f'{0:05}.npy'), robot_seg2)
        plt.clf()
            # Use RAFT to get optical flow
        optical_flow = get_pred_flow(obs_1, obs_n)
        optical_flow = optical_flow.squeeze().permute(1, 2, 0)

            # Save Flow in directories based on the gap
        flow_dir_name = f'FlowImages_gap{value}'
        flow_save_dir = os.path.join('tsg', flow_dir_name, category_name)
        os.makedirs(flow_save_dir, exist_ok=True)
        flow_image = flow_to_image(optical_flow.cpu().numpy())
        flow_image2 = cv2.resize(flow_image, (400, 400), interpolation=cv2.INTER_CUBIC) / 255
        plt.imsave(os.path.join(flow_save_dir, f'{0:05}.jpg'), flow_image2)
